...
double ADSRGenerator::sampleRate = 44100.0;

double ADSRGenerator::nextSample() {
...
void ADSRGenerator::calculateMultiplier(double startLevel,
                                            double endLevel,
                                            unsigned long long lengthInSamples) {
    multiplier = 1.0 + (log(endLevel) - log(startLevel)) / (lengthInSamples);
}

void ADSRGenerator::enterStage(EnvelopeStage newStage) {
...
void ADSRGenerator::setSampleRate(double newSampleRate) {
    sampleRate = newSampleRate;
}

void ADSRGenerator::setStageValue(EnvelopeStage stage,
                                      double value) {
