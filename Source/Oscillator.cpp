...
void Oscillator::setWaveType(OscillatorWaveType waveType) {
    mOscillatorWaveType = waveType;
}
...
void Oscillator::generate(double* buffer, int nFrames) {
    switch (mOscillatorWaveType) {
        ...
        case OSCILLATOR_WAVETYPE_SAW:
            ...
            break;
        case OSCILLATOR_WAVETYPE_SQUARE:
            for (int i = 0; i < nFrames; i++) {
                if (mPhase <= mPI) {
                    buffer[i] = 1.0;
                } else {
            ...
            break;
        case OSCILLATOR_WAVETYPE_TRIANGLE:
            for (int i = 0; i < nFrames; i++) {
...
double Oscillator::nextSample() {
    double value = naiveWaveformForWaveType(mOscillatorWaveType);
    mPhase += mPhaseIncrement;
    while (mPhase >= twoPI) {
        mPhase -= twoPI;
    }
...
double Oscillator::naiveWaveformForWaveType(OscillatorWaveType waveType) {
    double value;
    switch (waveType) {
        case OSCILLATOR_WAVETYPE_SINE:
            value = sin(mPhase);
